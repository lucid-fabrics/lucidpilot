...
def format_chrome_snapshot(snapshot: Any) -> str:
    ...
    lines.append(f"# Page snapshot{f' ({mode})' if mode else ''}")
...
def format_chrome_inspect(inspect: Any) -> str:
    ...
    if cs:
        lines.append(f'suggested click: my_browser_click({{ uid: "{cs.get("uid")}" }}) or x={cs.get("x")}, y={cs.get("y")}')
...
def summarize_action_result(result: Any) -> str | None:
    ...
    # glue.js's observed effects (layer 6). Preferred over upstream's coarse
    # pageMutated when present: this is a real MutationObserver count taken
    # around the action, not a heuristic, so it can say "nothing happened"
    # without the hedging the next branch needs. Computed since this layer
    # landed but not surfaced at first - the agent saw a bare "Clicked el-7"
    # and had no way to tell a real click from one that hit a spinner.
    effects = result.get("effects")
    if isinstance(effects, dict):
        if effects.get("navigated"):
            parts.append(f"navigated to {effects['navigated']}")
        elif effects.get("mutations") == 0:
            parts.append("changed nothing on the page")
    elif result.get("pageMutated") is False:
        parts.append("no coarse DOM change detected (may still have taken effect, verify with includeSnapshot)")

    # What a bare coordinate actually landed on, for the one target type that
    # cannot be verified beforehand.
    hit = result.get("hit")
    if isinstance(hit, dict):
        label = f' "{hit["text"]}"' if hit.get("text") else ""
        parts.append(f"landed on <{hit.get('tag') or '?'}{'#' + hit['id'] if hit.get('id') else ''}>{label}")
