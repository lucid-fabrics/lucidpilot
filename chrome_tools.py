"""The 21 my_browser_* tools: vendored from hermes-chrome-plugin's ``tools.py`` (its own
21 chrome_* tools; a Python port of ``registerChromeTools`` in
``pi-chrome/extensions/chrome-profile-bridge/index.ts``) and renamed to the
``my_browser_`` prefix so LucidPilot can be installed and loaded alongside the original
hermes-chrome-plugin with zero tool-name collisions. Behavior, schema, and
docstrings are otherwise unchanged; this is a rename, not a rewrite.

Handlers are sync (``handler(args, **kw) -> str``) and registered with a
``check_fn`` that requires BOTH ``auth.is_authorized()`` (Chrome control
granted) AND ``licensing.is_pro_licensed()`` (LucidPilot Pro subscription
active) so the tools stay invisible to the agent until both gates are open -
an unlicensed session sees neither these nor indicator_* (also gated). Each
handler also calls ``auth.require_authorized()`` and ``licensing.require_pro_licensed()``
before touching the bridge (defense in depth, same two-layer shape as the
visibility gate), shapes the wire params (background -> foreground), sends
through the bridge, and formats the result for the model.
"""
...
from .licensing import LicenseRequiredError, is_pro_licensed, require_pro_licensed

# The Chrome tab group LucidPilot drops its tabs into. Named for the product,
...
# may hold, in the place the model is actually looking when it chooses.
_REAL_CHROME_PREFIX = "[the user's own signed-in browser - their cookies, logins and open tabs]"
...
def _bg_prop() -> dict:
    return {
        "background": {
            "type": "boolean",
            "description": "If true (default), leave the tab where it is; false makes it the active tab inside Chrome so the user can watch. Never raises the Chrome window or takes focus from the user's terminal either way.",
        }
    }
...
def _snapshot_extra_props() -> dict:
    return {
        "includeSnapshot": {"type": "boolean", "description": "Include a fresh my_browser_snapshot of the page after the action."},
...
def _send(auth: ChromeAuth, bridge: ChromeProfileBridge, action: str, params: dict, timeout_ms: int = DEFAULT_TIMEOUT_MS) -> Any:
    # Every my_browser_* handler funnels through here, so this is the one place both
    # runtime gates need to live (root cause, not one guard per handler).
    ...
    if action.startswith("page.") and "sessionGroupTitle" not in params:
        params = {**params, "sessionGroupTitle": GROUP_TITLE, "joinSessionGroup": True}
    # Tells glue.js which agent to label the overlay toast/log with, and which
    # accent to tint it (see bridge.AGENT). Without this every action showed
    # "Hermes" in blue-violet even when Claude Code was driving.
    #
    # sessionId (see bridge.SESSION_ID) is how glue.js keeps two independent
...
def register_all_tools(ctx, bridge: ChromeProfileBridge, auth: ChromeAuth) -> None:
    def _authorized_and_licensed() -> bool:
        # Visibility layer: my_browser_* tools stay out of the agent's context unless
        # BOTH gates are open. Mirrors auth.is_authorized's own check_fn
        # contract (cheap, side-effect-light, no network) - is_pro_licensed()
        # is documented as safe for exactly this on that same basis.
        return auth.is_authorized() and is_pro_licensed()
    ...
    def add(
        ...
        *,
        ...
        # (default ""); forwarding it is what lets a non-Hermes host (the MCP
        # server) surface per-tool descriptions instead of bare names.
        #
        # entry_point marks the tools a model reaches for FIRST once it decides
        # to use a browser at all. Those carry the differentiator inline: a
        # session can hold three browser stacks (the builtin sandbox browser_*,
        # hermes-chrome-plugin's chrome_*, and these), and a bare "Navigate a
        # tab to a URL" is indistinguishable from the other two at exactly the
        # moment the choice gets made. The primer says the same thing, but the
        # primer is paragraphs away while the description sits right there -
        # and smaller models lean on descriptions far more. Deliberately not
        # stamped on all 21: the follow-up tools get chosen because the entry
        # point already was, so the repetition would only cost context.
        if entry_point:
            description = f"{_REAL_CHROME_PREFIX} {description}"
        ctx.register_tool(
            name=name,
            toolset="lucidpilot",
            schema={"type": "object", "properties": properties, "required": required or []},
            handler=_guard(handler),
            check_fn=_authorized_and_licensed,
    ...
    add(
        ...
        "Start/check the local bridge used by the companion Chrome extension. Does not launch a separate Chrome profile; load the unpacked chrome-extension folder in your existing Chrome to connect.",
        {
            ...
            "port": {"type": "number", "description": "Ignored. The bundled extension polls 127.0.0.1:16329."},
            ...
            "background": {
                "type": "boolean",
                "description": "Only meaningful with url set: if true (default), the new tab opens without becoming active or disturbing what the user is looking at; false makes it the active tab so the user can watch. Never raises the Chrome window or takes focus from the user's terminal either way.",
            },
        },
        h_launch,
        entry_point=True,
    ...
    def h_tab(args: dict) -> str:
        ...
        # Only action=new needs the foreground/background translation - a new
        # tab is created active by default in a normal browser, exactly the
        # disruption background=true (the same default every other action
        # already uses) exists to avoid. Other tab actions (list/activate/
        # close/...) have no such default to fight, so leave them as-is.
        forwarded = _wire(bridge, args) if action == "new" else _clean(args)
        if action in ("new", "group") and forwarded.get("groupTitle") is None and args.get("group") is not False:
            forwarded["groupTitle"] = GROUP_TITLE
    ...
    add(
        ...
        "List, create, activate, close, group, ungroup, or inspect tabs in the user's existing Chrome profile.",
        {
            ...
            "background": {
                "type": "boolean",
                "description": "action=new only: if true (default), the new tab opens without becoming active or disturbing what the user is looking at; false makes it the active tab so the user can watch. Never raises the Chrome window or takes focus from the user's terminal either way.",
            },
        },
        h_tab,
        required=["action"],
    ...
    add(
        "my_browser_find",
        "Find elements, page regions, or text on the current Chrome page by query. Returns ranked matches with stable uids and coordinates. A focused wrapper around my_browser_snapshot({query}).",
        {
    ...
    def h_inspect(args: dict) -> str:
        try:
            ...
            return (
                "my_browser_inspect fallback: loaded Chrome extension does not yet support page.inspect; "
                "reload it at chrome://extensions for deep inspect.\n\n" + format_chrome_snapshot(snapshot)
            )

    add(
        "my_browser_inspect",
        "Inspect one snapshot uid or selector deeply: nearby text, nearby actions, form context, ancestors, and suggested click target. Use after my_browser_snapshot/my_browser_find.",
        {
            "uid": {"type": "string", "description": "Stable element uid from my_browser_snapshot/my_browser_find."},
    ...
    add(
        ...
        {
            "uid": {"type": "string", "description": "Stable element uid from my_browser_snapshot. Prefer uid over selector."},
    ...
    add(
        ...
        "Retrieve one captured fetch/XMLHttpRequest entry (including response body when available) by requestId from my_browser_list_network_requests.",
        {"requestId": {"type": "string", "description": "Request id from my_browser_list_network_requests."}, **_target_props(), **_bg_prop()},
